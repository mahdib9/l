...
local function check_member_autorealm(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'به گروه مدیریت جدید خوش آمدید')
    end
  end
end
local function check_member_realm_add(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      data[tostring(realms)][tostring(msg.to.id)] = msg.to.id
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'گروه مدیریت به سرور افزوده شد')
    end
  end
end
...
local function check_member_modadd(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      data[tostring(groups)][tostring(msg.to.id)] = msg.to.id
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'گروه به سرور افزوده گردید و شما به عنوان صاحب گروه شناخته شدید')
    end
  end
end
...
local function check_member_realmrem(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'این گروه مدیریت از سرور پاک گردید')
    end
  end
end
local function check_member_modrem(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'این گروه از سرور پاک  گردید ')
    end
  end
end
--End Check Member
local function show_group_settingsmod(msg, data, target)
 	if not is_momod(msg) then
    	return "شما دسترسی دریافت تنظیمات را ندارید"
  	end
  	local data = load_data(_config.moderation.data)
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'زبان فارسی قفل نیست'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل زبان فارسی غیرفعال شد\nهم اکنون زبان فارسی مجاز است'
  end
end
...
local function unlock_group_english(msg, data, target)
  ...
  if group_english_lock == 'no' then
    return 'زبان انگلیسی مجاز است'
  else
    data[tostring(target)]['settings']['lock_english'] = 'no'
    save_data(_config.moderation.data, data)
    return 'زبان انگلیسی مجاز شد'
  end
end
...
local function lock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'yes' then
    return 'تگ کردن متن قفل است'
  else
    data[tostring(target)]['settings']['antitag'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تگ کردن متن قفل شد'
  end
end
...
local function unlock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'no' then
    return 'تگ کردن متن مجاز است'
  else
    data[tostring(target)]['settings']['antitag'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تگ کردن متن مجاز شد'
  end
end
...
local function lock_group_join(msg, data, target)
  ...
  if group_join_lock == 'yes' then
    return 'ورود با لینک ممنوع است'
  else
    data[tostring(target)]['settings']['lock_join'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'ورود با لینک ممنوع شد'
  end
end
...
local function lock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'yes' then
    return 'الفاظ رکیک ممنوع است'
  else
    data[tostring(target)]['settings']['antifosh'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'الفاظ رکیک ممنوع شدند'
  end
end
...
local function unlock_group_fosh(msg, data, target)
  ...
  if group_fosh_lock == 'no' then
    return 'الفاظ رکیک مجاز است'
  else
    data[tostring(target)]['settings']['antifosh'] = 'no'
    save_data(_config.moderation.data, data)
    return 'الفاظ رکیک مجاز شد'
  end
end
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return 'محافظ ربات فعال است'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'محافظ ربات فعال شد'
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return 'محافظ ربات غیرفعال است'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return 'محافظ ربات غیرفعال شد'
  end
end
...
local function lock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'yes' then
    return 'تعویض نام گروه ممنوع است'
  else
    data[tostring(target)]['settings']['lock_name'] = 'yes'
    save_data(_config.moderation.data, data)
    rename_chat('chat#id'..target, group_name_set, ok_cb, false)
    return 'تعویض نام گروه ممنوع شد'
  end
end
local function unlock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'no' then
    return 'تعویض نام گروه مجاز است'
  else
    data[tostring(target)]['settings']['lock_name'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تعویض نام گروه آزاد شد'
  end
end
...
local function unlock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'ضد اسپم غیرفعال است'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return 'ضد اسپم غیرفعال شد\nهم اکنون اسپم کجاز است'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'عضوگیری گروه ممنوع است'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'عضوگیری گروه ممنوع شد'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'عضوگیری گروه مجاز است'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'عضوگیری گروه مجاز شد'
  end
end
...
local function lock_group_leave(msg, data, target)
  ...
  if leave_ban == 'yes' then
    return 'هم اکنون مسدودسازی کاربران خارج شده فعال است'
  else
    data[tostring(msg.to.id)]['settings']['leave_ban'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'مسدودسازی کاربران خارج شده فعال شد'
end
...
local function unlock_group_leave(msg, data, target)
  ...
  if leave_ban == 'no' then
    return 'مسدودسازی کاربران خارج شده غیرفعال است'
  else
    data[tostring(msg.to.id)]['settings']['leave_ban'] = 'no'
    save_data(_config.moderation.data, data)
    return 'مسدودسازی کاربران خارج شده غیرفعال شد'
  end
end
...
local function unlock_group_photomod(msg, data, target)
  ...
  if group_photo_lock == 'no' then
    return 'تعویض عکس گروه ممنوع است'
  else
    data[tostring(target)]['settings']['lock_photo'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تعویض عکس گروه ممنوع شد'
  end
end
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return 'این قوانین برای گروه ثبت شدند\n_______________\n\n'..rules
end
local function modadd(msg)
  ...
  if is_group(msg) then
    return 'این گروه در سرور افزوده شده بود'
  end
    receiver = get_receiver(msg)
...
local function realmadd(msg)
  ...
  if is_realm(msg) then
    return 'این گروه مدیریت در سرور افزوده شده بود'
  end
    receiver = get_receiver(msg)
...
function realmrem(msg)
  -- superuser and admins only (because sudo are always has privilege)
  if not is_admin(msg) then
    return "شما دسترسی پاک کردن گروه مدیریت از سرور را ندارید"
  end
  local data = load_data(_config.moderation.data)
  if not is_realm(msg) then
    return 'گروه مدیریت به سرور افزوده نشده است'
  end
    receiver = get_receiver(msg)
...
local function set_group_photo(msg, success, result)
  ...
  if success then
    ...
    save_data(_config.moderation.data, data)
    send_large_msg(receiver, 'عکس گروه ذخیره و قفل شد', ok_cb, false)
  else
...
local function demote(receiver, member_username, member_id)
  ...
  save_data(_config.moderation.data, data)
  return send_large_msg(receiver, member_username..' از مدیریت گروه برکنار شد')
end
...
local function setowner_by_reply(extra, success, result)
  ...
  data[tostring(msg.to.id)]['set_owner'] = tostring(msg.from.id)
      ...
      local text = msg.from.print_name:gsub("_", " ").." هم اکنون صاحب گروه شد"
...
local function modlist(msg)
  ...
  local i = 1
  local message = '\nلیست مدیران گروه در ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n_______________\n'
...
local function run(msg, matches)
  ...
  if matches[1] == 'add' and not matches[2] then
    if is_realm(msg) then
       return 'خطا: اینجا به عنوان گروه مدیریت تنظیم شده است'
    end
   ...
   if matches[1] == 'add' and matches[2] == 'realm' then
    if is_group(msg) then
       return 'خطا: اینجا به عنوان گروه تنظیم شده است'
    end
    print("گروه  "..msg.to.print_name.."("..msg.to.id..") به عنوان گروه مدیریت افزوده شد")
    return realmadd(msg)
  end
  if matches[1] == 'rem' and not matches[2] then
    print("گروه  "..msg.to.print_name.."("..msg.to.id..")  پاک شد")
    return modrem(msg)
  end
  if matches[1] == 'rem' and matches[2] == 'realm' then
    print("گروه "..msg.to.print_name.."("..msg.to.id..") از لیست گروه های مدیریت پاک شد")
    return realmrem(msg)
  end
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'setphoto' and is_momod(msg) then
      ...
      save_data(_config.moderation.data, data)
      return 'عکس جدید را برای قفل و ثبت روی پروفایل گروه ارسال کنید'
    end
    if matches[1] == 'promote' and not matches[2] then
      if not is_owner(msg) then
        return "شما دسترسی مدیرکردن فرد را ندارید"
      end
    ...
    if matches[1] == 'promote' and matches[2] then
      ...
      if not is_owner(msg) then
        return "تنها صاحب گروه دسترسی مدیر کردن فرد را دارد"
      end
    ...
    if matches[1] == 'demote' and not matches[2] then
      if not is_owner(msg) then
        return "شما دسترسی برکنار کردن از مدیریت را ندارید"
      end
      if type(msg.reply_id)~="nil" then
    ...
    if matches[1] == 'demote' and matches[2] then
      ...
      if not is_owner(msg) then
        return "تنها صاحب گروه دسترسی برکنار کردن مدیران را دارد"
      end
      if string.gsub(matches[2], "@", "") == msg.from.username and not is_owner(msg) then
        return "شما نمیتوانید خود را از مدیریت برکنار کنید"
      end
	local member = matches[2]
  ...
  --[[if matches[1] == 'public' then
    ...
    if matches[1] == 'newlink' and not is_realm(msg) then
      ...
      local function callback (extra , success, result)
        ...
        if success == 0 then
           return send_large_msg(receiver, '*خطادر دریافت لینک* \nعلت: ربات سازنده گروه نیست.')
        end
    ...
    if matches[1] == 'setowner' and matches[2] then
      if not is_owner(msg) then
        return "شما دسترسی انتخاب صاحب گروه را ندارید"
      end
    ...
    if matches[1] == 'setflood' then 
      ...
      if tonumber(matches[2]) < 5 or tonumber(matches[2]) > 100 then
        return "خطا,محدوده اعداد بین 5 تا 100 میباشد"
      end
      local flood_max = matches[2]
      data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
      save_data(_config.moderation.data, data)
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
      return 'مقدار حساسیت اسپم تنظیم شد به '..matches[2]
    end
    if matches[1] == 'clean' then
      ...
      if matches[2] == 'modlist' then
        if next(data[tostring(msg.to.id)]['moderators']) == nil then --fix way
          return 'مدیری ثبت نشده است'
      else 
          return 'مدیران گروه پاک شدند'
        end
        local message = '\nلیست مدیران گروه در ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n_______________\n'
        for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
      ...
      if matches[2] == 'rules' then 
        ...
        send_large_msg("chat#id".. msg.to.id , "قوانین گروه پاک شد\nهم اکنون با دستور \n!set rules TEXT\n میتوانید قوانین جدید را ثبت کنید")
        save_data(_config.moderation.data, data)
      ...
      if matches[2] == 'about' then 
        local data_cat = 'description'
        data[tostring(msg.to.id)][data_cat] = nil
                    send_large_msg("chat#id".. msg.to.id , "توضيحات گروه پاک شد\nهم اکنون با دستور \n!set about TEXT\n ميتوانيد توضيحات جديد رو ثبت کنيد")
        save_data(_config.moderation.data, data)
    ...
    if matches[1] == 'kill' and matches[2] == 'chat' then
      if not is_admin(msg) then
          return 'شما دسترسی مصادره گروه را ندارید'
      end
      if not is_realm(msg) then
          local receiver = get_receiver(msg)
          return modrem(msg),
          ...
          return 'اینجا گروه مدیریت است'
      end
   end
    if matches[1] == 'kill' and matches[2] == 'realm' then
     if not is_admin(msg) then
         return 'شما دسترسی مصادره گروه مدیریت را ندارید'
     end
     if not is_group(msg) then
        local receiver = get_receiver(msg)
        return realmrem(msg),
    ...
    if matches[1] == 'kickinactive' then
      --send_large_msg('chat#id'..msg.to.id, 'I\'m in matches[1]')
	    if not is_momod(msg) then
	      return 'شما دسترسی جذف غیرفعالان را ندارید'
	    end
	    local num = 1
	    if matches[2] then
	        num = matches[2]
