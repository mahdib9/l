local filename='data/expire.lua'
 ...
 local function cron()
   for date, values in pairs(cronned) do
   	if date < os.time() then --time's up
 	  	send_msg(values[1][1], "مدت زمان گروه شما به پایان رسیده است\n\nبه چنل مراجعه نموده و با وارد شدن به چنل گروه خود را تمدید کنید\n__________________\nلینک چنل: \n @HEXTOR_CH"..values[1][2], ok_cb, false)
   		delete_cron(date) 
 	end
 ...
 local function actually_run(msg, delay,text)
   if (not delay or not text) then
   	return "روش انجام:  !expire [delay: 2h3m1s] text"
   end
   save_cron(msg, text,delay)
   return "تاریخ انتضای گروه به \n  " .. os.date("%x \n ساعت   %H:%M:%S",delay) .. "\nتنظیم شد\n_________________\n توضیحات\n_________________\n " .. text .. ""
 end
